...
locations = []
class Location:
	def __init__(self, x, y, row_size, column_size, height, is_start):
		...
		self.visited = False
	def check_surroundings(self): # return cost of a if found, otherwise Null
		global locations
		...
		for direction in cardinalities:
			...
			if not locations[index].visited and locations[index].cost_to_reach > self.cost_to_reach+1 and self.height <= locations[index].height + 1:
				locations[index].cost_to_reach = self.cost_to_reach+1
				if locations[index].height<=ord("a"):
...
for y, line in enumerate(lines):
	for x, value in enumerate(line.strip()):
		...
		if value == "E":
			height = ord("z")
		locations.append(Location(x,y,len(line.strip()),len(lines),height,value == "E"))
...
while True:
	...
	if cost_of_end:
		print("{} steps to reach a point with height of 0".format(cost_of_end))
		break
